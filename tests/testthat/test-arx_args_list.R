test_that("arx_args checks inputs", {
  ...
  expect_error(arx_args_list(quantile_values = -.1))
  expect_error(arx_args_list(quantile_values = 1.1))
  expect_type(arx_args_list(quantile_values = NULL), "list")
...
test_that("arx forecaster disambiguates quantiles", {
  alist <- eval(formals(arx_args_list)$quantile_values)
  tlist <- eval(formals(quantile_reg)$quantile_values)
