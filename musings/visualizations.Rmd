...
Often times on the news, you will see charts of a time series that will
illustrate trends of COVID. Given limitations on the number of tests available,
this may not be fully accurate.

Note that while it may preferable to convert this data into an `epi_df`,
we want to keep it tidy (i.e. one observation per row) as to facilitate its
use with `ggplot`, and having converting it to an `epi_df` rather than using
`gather` to make it tidy will take out this property.

```{r}
hosp <- read_csv("https://raw.githubusercontent.com/ccodwg/CovidTimelineCanada/main/data/pt/hospitalizations_pt.csv")
icu <- read_csv("https://raw.githubusercontent.com/ccodwg/CovidTimelineCanada/main/data/pt/icu_pt.csv")

hosp_icu <- inner_join(hosp,icu,by=c("region","date")) %>%
  select(region, date,hospitalizations = value.x, icu = value.y) %>%
  gather("type","value",-region,-date)

hosp_icu2 <- filter(hosp_icu,region %in% c("BC","ON","QC","AB"))

ggplot(hosp_icu2,aes(x=date,y=value,colour=type)) +
  geom_line() +
  facet_wrap(~region)
```

This is a time series for the hospitalizations and ICU admissions of the four
most populated provinces in Canada.
