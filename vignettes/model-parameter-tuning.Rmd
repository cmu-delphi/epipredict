...
hyperparameter, it can be rather cumbersome and not fun to comb through.
...
`select_best()` to select the hyperparameters with the best results 
numerically.

```{r}
show_best(grid_res, metric = "rmse")

select_best(grid_res, metric = "rmse")
```

Now, we'll specify these parameters in a tibble and then use 
the `finalize_workflow()` function to integrate these into 
our workflow:

```{r}
linear_param <- tibble(penalty = 0.000000377, mixture = 0.259)
...
We are now all set to fit the model to the training set and 
to use that to make predictions.
