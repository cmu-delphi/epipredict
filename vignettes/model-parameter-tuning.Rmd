...
In this vignette, we're going to look at using k-fold cross validation to compare which of two models is more useful in predicting death rates and then we'll do some hyperparameter tuning on the chosen model.

The dataset we'll be using is `case_death_rate_subset`, which contains confirmed COVID-19 cases and deaths from Dec 31, 2020 to Dec 31, 2021 from reports made available by Johns Hopkins University. To simplify things, we'll just use the data for California from October 1, 2021 to December 31, 2021, inclusive:
...
in handy for doing time series cross-validation. We will illustrate using that 
function below to get the cross-validation folds:
...
In total, there are `nrow(folds)` folds. And we can do a sanity check of the date range 
...
has a lower mean squared prediction error (MSPE). We will go for that model
with the lower MSPE.
...
To do this, we will create a random grid of parameter combinations. The size
parameter controls the number of parameter combinations returned in the 
random grid.
