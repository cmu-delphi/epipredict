...
get_prediction <- function(model, assessment_date, recipe, data){
  ...
  assess <- preprocessed %>% 
    ...
    select(yardstick::tidy(model)$term[-1], time_value, geo_value)

  return(predict(model$fit$fit, new_data = assess))
}

assessment_date_list <- map(lapply(folds$splits, assessment),
                  ~ pluck( . ,"time_value")) 

assessment_date <- do.call("c", assessment_date_list)

pred_less <- map2(models_less, assessment_date, get_prediction,
             data = x, recipe = r_less) 
pred_more <- map2(models_more, assessment_date, get_prediction,
