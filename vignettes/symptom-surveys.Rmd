...
geo_values_complete <- intersect(
  intersect(g$geo_value, f$geo_value),
  c$geo_value
)
...
for (k in 1:length(dates)) {
  ...
  # Pre-structuring test data for this date
  z_te <- z %>%
    rename(
...
res_all4 <- out_df %>%
  ...
  mutate(
    err1 = err1 / err0, err2 = err2 / err0, # Compute relative error
    err3 = err3 / err0, err4 = err4 / err0
  ) %>% # to strawman model
  mutate(
    dif12 = err1 - err2, dif13 = err1 - err3, # Compute differences
    dif14 = err1 - err4
...
case_fb_mods <- function(forecast_dates, leads) {
  ...
  for (k in 1:length(forecast_dates)) {
    ...
    # Strawman model
    out_df0 <- map(leads, ~ flatline_forecaster(
      z %>% filter(between(time_value, date - .x - max(lags) - n, date)) %>% select(time_value, geo_value, case),
      outcome = "case",
      args_list = arx_args_list(
        lags = lags,
        ahead = .x,
    ...
    # Cases model
    out_df1 <- map(leads, ~ arx_forecaster(
      z %>% filter(between(time_value, date - .x - max(lags) - n, date)) %>% select(time_value, geo_value, case) %>% filter(complete.cases(.)),
      outcome = "case",
      predictors = "case",
      trainer = quantile_reg(),
      args_list = arx_args_list(
    ...
    # Cases and Facebook model
    out_df2 <- map(leads, ~ arx_forecaster(
      z %>% filter(between(time_value, date - .x - max(lags) - n, date)) %>% select(time_value, geo_value, case, fb) %>% filter(complete.cases(.)),
      outcome = "case",
      predictors = c("case", "fb"),
      trainer = quantile_reg(),
      args_list = arx_args_list(
...
This chapter was adapted from the following [Delphi blog post](https://delphi.cmu.edu/blog/2020/09/21/can-symptoms-surveys-improve-covid-19-forecasts/), with the necessary modifications to enable the use of `epipredict`. Note that the results may be different from those on the blog post (one reason is that we are exploring the use of a different forecaster and another is that we're using the most recent versions of the datasets).
