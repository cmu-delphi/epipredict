...
out_q <- arx_forecaster(jhu, "death_rate", c("case_rate", "death_rate"),
  args_list = arx_args_list(
    quantile_values = c(.01, .025, seq(.05, .95, by = .05), .975, .99)
  )
)
```
...
or extract the entire distribution into a "long" `epi_df` with `quantile_values`
...
arx_args_list(
  ...
  forecast_date = NULL, target_date = NULL, quantile_values = c(0.05, 0.95),
