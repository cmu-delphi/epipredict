...
deaths_incidence_num <- pub_covidcast(
  source = "jhu-csse",
...
x <- epix_merge(
      epix_merge(confirmed_incidence_num, deaths_incidence_num,
                 sync = "locf", compactify = FALSE),
      epix_merge(confirmed_incidence_prop, deaths_incidence_prop,
                 sync = "locf", compactify = FALSE),
      sync = "locf",
      compactify = TRUE
)
...
# computing the 7-day average of the rates
x <- x %>%
  epix_slide(
  ...
  rename(time_value = slide_value_time_value, 
         geo_value = slide_value_geo_value,
         cases = slide_value_cases,
         deaths = slide_value_deaths,
         case_rate = slide_value_case_rate,
         death_rate = slide_value_death_rate,
         case_rate_7d_av = slide_value_case_rate_7d_av,
         death_rate_7d_av = slide_value_death_rate_7d_av) %>% as_epi_archive()
```
...
x_result <- x %>%
  epix_slide(forecaster, before = 120, ref_time_values = fc_time_values,
            names_sep = NULL) %>%
  mutate(engine_type = engine$engine) %>%
  as_epi_df() 
