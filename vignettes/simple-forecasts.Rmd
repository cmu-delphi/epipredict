...
y1 <- covidcast(
  data_source = "doctor-visits", signals = "smoothed_adj_cli",
  time_type = "day", geo_type = "state",
  time_values = epirange(20200601, 20211201),
  geo_values = "ca,fl", issues = epirange(20200601, 20211201)
) %>% fetch_tbl()
...
epix_merge(
  x, 
  y2 %>% select(geo_value, time_value, version = issue, case_rate = value) %>% as_epi_archive(),
  all = TRUE
)
...
$$
\min_\Gamma \lVert \mathbf{Y}\mathbf{H} - \mathbf{X}\Gamma \rVert_F^2.
$$
Predictions can then be produced easily by undoing the transformation with $\mathbf{H}^\mathsf{T}$. See [Tuzhilina et al.](https://arxiv.org/abs/2202.09723) for more details.
...
can <- readRDS(system.file("extdata", "can_prov_cases.rds", package = "epipredict", mustWork = TRUE)) %>%
...
can_fc <- suppressWarnings(purrr:::map_dfr(c(7,14,21,28), ~ can_k_week_ahead(.x)))
```
...
ggplot(can_fc, aes(x = target_date, group = time_value)) +
  ...
  geom_line(data = can_latest,
            aes(x = time_value, y = cr_7dav),
            inherit.aes = FALSE, color = "gray50") +
