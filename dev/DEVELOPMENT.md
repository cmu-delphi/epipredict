...
Note that sometimes the caches from either `pkgdown` or `knitr` can
cause difficulties. To clear those, run `make`, with either
`clean_knitr`, `clean_site`, or `clean` (which does both).

If you work without R Studio and want to iterate on documentation, you
might find `Rscript pkgdown-watch.R` useful. helpful. For updating
references, you will need to manually call
[`pkgdown::build_reference()`](https://pkgdown.r-lib.org/reference/build_reference.html).

## Versioning

Please follow the guidelines in the [PR template
document](https://cmu-delphi.github.io/epipredict/dev/pull_request_template.md).

## Planned CRAN release process

Open a release issue and then copy and follow this checklist in the
issue (modified from the checklist generated by
...
results](https://cran.rstudio.org/web/checks/check_results_epipredict.html).

`devtools::check(".", manual = TRUE, env_vars = c(NOT_CRAN = "false"))`.

- Aim for 10/10, no notes.

If check works well enough, merge to main. Otherwise open a PR to fix
up.

[Polish
NEWS](https://github.com/cmu-delphi/epipredict/blob/dev/NEWS.md).
...
`usethis::use_github_release(publish = FALSE)` (publish off, otherwise
it won’t push) will create a draft release based on the commit hash in
CRAN-SUBMISSION and push a tag to the GitHub repo.
