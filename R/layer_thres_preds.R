...
#' f <- frosting() %>%
#'   layer_predict() %>%
#'   layer_thres_preds(pred_lower = 0.180, pred_upper = 0.310)
...
layer_thres_preds_new <- function(pred_lower, pred_upper, id = rand_id("thres_preds")) {
  layer("thres_preds", pred_lower = pred_lower, pred_upper = pred_upper, id = id)
}

#' @export
slather.layer_thres_preds <- function(object, components, the_fit, ...) {
